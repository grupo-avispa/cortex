...
DSRGraph::DSRGraph(std::string name, uint32_t id, const std::string &dsr_input_file, const std::string &partition_name, bool all_same_host)
        : agent_id(id),
        ...
        generator(id),
        domain_partition(partition_name)
{
    ...
    auto [res, pub, writer] = dsrpub_node.init(participant_handle, dsrparticipant.getNodeTopic(),domain_partition);
    auto [res2, pub2, writer2] = dsrpub_node_attrs.init(participant_handle, dsrparticipant.getAttNodeTopic(),domain_partition);

    auto [res3, pub3, writer3] = dsrpub_edge.init(participant_handle, dsrparticipant.getEdgeTopic(),domain_partition);
    auto [res4, pub4, writer4] = dsrpub_edge_attrs.init(participant_handle, dsrparticipant.getAttEdgeTopic(),domain_partition);

    auto [res5, pub5, writer5] = dsrpub_graph_request.init(participant_handle, dsrparticipant.getGraphRequestTopic(),domain_partition);
    auto [res6, pub6, writer6] = dsrpub_request_answer.init(participant_handle, dsrparticipant.getGraphTopic(),domain_partition);
    ...
    if (inserted)
    {
        if (!copy)
        {
            if (delta.has_value())
            {
                ...
                emit update_node_signal_by_node(node, SignalInfo{agent_id});
                emit create_node_signal(node.id(), node.type(), SignalInfo{agent_id});
                emit create_node_signal_by_node(node, SignalInfo{agent_id});
    ...
    if (updated) {
        if (!copy) {
            if (vec_node_attr.has_value()) {
                ...
                emit update_node_attr_signal_by_node(node, atts_names, SignalInfo{agent_id});
            }
        }
    }
    return updated;
        ...
        if (joined) {
            if (signal) {
                ...
                for (const auto &[k, v] : nodes.at(id).read_reg().fano()) {
                    ...
                    /*
                    Edge JP_edge(v.read_reg());
                    emit update_edge_signal_by_edge(JP_edge, SignalInfo{ mvreg.agent_id() });
                    std::cout << "FANO CHECKCHECKCHECK: " << std::endl;
                    std::cout << "-----> JP_edge: from: " << JP_edge.from()<< " , to: " << JP_edge.to() << " , type: "<< JP_edge.type()<< std::endl;
                    std::cout << "-----> UEXedge: from: " << id<< " , to: " << k.first << " , type: "<< k.second<< std::endl;
                    */
                ...
                for (const auto &[k, v]: map_new_to_edges)
                {
                    ...
                    auto edg = get_edge(k, id, v);
                    if (edg.has_value()) {
                        emit update_edge_signal_by_edge(edg.value(), SignalInfo{ mvreg.agent_id() });
                    } else {
                        std::cout << "[JOIN NODE] WARNING!!! update_edge_signal_by_edge not emitted to add edge TO: "<< k << ", " << id << ", " << v << std::endl;
                    }
                }
            } else {
...
void DSRGraph::join_delta_edge(IDL::MvregEdge &&mvreg)
{
    try {
        ...
        auto edg_to_process = get_edge(from, to, type); // edge to delete
        ...
        std::optional<Edge> deleted_edge;
        {
            ...
            if (cfrom and cto) {
                ...
                if (!find) {
                    ...
                    qDebug() <<"TODO: Unhandled, we should sync the deleted_nodes set";
                }
            } else {
                qDebug() << "THE EDGE IS PART OF A DELETED NODE, CLEAN FROM UNPROCESSED";
        ...
        if (joined) {
            if (signal) {
                ...
                edg_to_process = get_edge(from, to, type); // edge to add
...
void DSRGraph::node_subscription_thread(bool showReceived)
{
    ...
    auto [res, sub, reader] = dsrsub_node.init(dsrparticipant.getParticipant(), dsrparticipant.getNodeTopic(), dsrpub_call_node, mtx_entity_creation, domain_partition);
...
void DSRGraph::edge_subscription_thread(bool showReceived)
{
    ...
    auto [res, sub, reader]  = dsrsub_edge.init(dsrparticipant.getParticipant(), dsrparticipant.getEdgeTopic(), dsrpub_call_edge, mtx_entity_creation, domain_partition);
...
void DSRGraph::edge_attrs_subscription_thread(bool showReceived)
{
                    ...
                    if (m_info.instance_state == eprosima::fastdds::dds::ALIVE_INSTANCE_STATE) {
                        ...
                        if (!samples.vec().empty() and samples.vec().at(0).agent_id() != agent_id)
                        {
                            tp_delta_attr.spawn_task([this, samples = std::move(samples)]() mutable {
                                ...
                                auto edg = get_edge(from, to, type);
                                if (edg.has_value()) {
    ...
    auto [res, sub, reader] = dsrsub_edge_attrs.init(dsrparticipant.getParticipant(), dsrparticipant.getAttEdgeTopic(),
                           dsrpub_call_edge_attrs, mtx_entity_creation, domain_partition);
...
void DSRGraph::node_attrs_subscription_thread(bool showReceived)
{
    ...
    auto [res, sub, reader] = dsrsub_node_attrs.init(dsrparticipant.getParticipant(), dsrparticipant.getAttNodeTopic(),
                           dsrpub_call_node_attrs, mtx_entity_creation, domain_partition);
...
void DSRGraph::fullgraph_server_thread()
{
    ...
    auto [res, sub, reader] = dsrsub_graph_request.init(dsrparticipant.getParticipant(), dsrparticipant.getGraphRequestTopic(),
                              dsrpub_graph_request_call, mtx_entity_creation, domain_partition);
...
std::pair<bool, bool> DSRGraph::fullgraph_request_thread()
{
    ...
    auto [res, sub, reader] = dsrsub_request_answer.init(dsrparticipant.getParticipant(), dsrparticipant.getGraphTopic(),
                               dsrpub_request_answer_call, mtx_entity_creation, domain_partition);
...
DSRGraph::DSRGraph(const DSRGraph &G) : agent_id(G.agent_id), copy(true), tp(1), generator(G.agent_id)
{
    ...
    domain_partition = G.domain_partition;
}
